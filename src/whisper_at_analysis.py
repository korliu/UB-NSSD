...
if not os.path.exists(OUTPUT_DIR):
    os.mkdir(OUTPUT_DIR)
OUTPUT_FILENAME = "whisper_at_analysis.csv"
...
PRIO_LABELS = [54,55] # [(indices of labels we want)] ex: [54, 55] to see only biting/chewing labels (their label ids)
...
labels = ['food', 'speaker', 'audio_path', 'event_num', 
          ...
          'expected_tag', 'detected_tags', 'contains_correct_tag']
...
def add_to_datarow(
        ...
        expected_tag: str, detected_tags: list, contains_correct_tag: bool) -> None:
    ...
    data_dict['contains_correct_tag'].append(contains_correct_tag)
        ...
        for data_file in os.scandir(speaker_samples):
            ...
            with open(annotated_data,'r',encoding='utf-8') as f:
                ...
                for e in audio_events:
                    ...
                    torchaudio.save(temp_audio_path,event_audio,audio_sr)
                    ...
                    # collect all tags
                    correct_tag = True
                    flatten_tags = [tag for segment in parsed_results for tag in segment['audio tags']]
                    ...
                    flatten_tags = sorted(flatten_tags,key=lambda x: x[1], reverse=True)
                    # grab tags only and remove dupes
                    flatten_tags = list(set([t[0] for t in flatten_tags]))
                    
                    print(f"Tags for \"{annotated_data}\", Event #{event_num}: {flatten_tags}")
                    ...
                    add_to_datarow(
                        ...
                        expected_tag=l2e[str(event_label)],detected_tags=flatten_tags,contains_correct_tag=correct_tag
                    )
                    files_scanned += 1


# remove the temp audio file
if os.path.exists(os.path.join("src","temp_audio.wav")):
    os.remove(path=os.path.join("src","temp_audio.wav")) 
