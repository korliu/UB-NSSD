...
id2name, id2mid, mid2id, name2id = get_label2name_dict(
    os.path.join(OUTPUT_FOLDER, "audioset_labels.csv")
)
...
for k, v in dataset_links.items():
    ...
    with open(
        os.path.join(OUTPUT_DATA_FOLDER, f"{dataset_name}.csv"), "w+", newline=""
    ) as f, open(ALL_DATA_PATH, "a+", newline="") as f_all:
...
with open(audioset_data_path, "a", newline="") as audioset_data, open(
    manual_data_path, "r"
) as manual_data:
